...
_Avoid_: an escape field on `x_chartagent`, `ChartDocument` as the stored artifact, `bind_recipe` returning `BoundDocument`, a wire format for `BoundRecipe`, a sixth table, calling it a frame or a spec, a `semantic_types` field on `ChartRecipe` (they stay on the in-request `GeneratedRecipe`)
