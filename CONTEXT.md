...
**Escape reason**:
Why a request left the deterministic rail — one of **four** buckets, each naming a different lever: *no chart type in the 48* (upstream, not ours), *the transform menu cannot express it* (grow the menu), *an expressible frame was escaped anyway* (planner quality), and *expressible, produced, failed review* (the gate's own numbers). The planner writes the first three at plan time; the **gate** writes the fourth at escalation. Buckets 1 and 2 carry one closed lever-naming field; 3 and 4 carry none. **Where it is written is settled**: `ChartRecipe.escape_reason`, required on every recipe and carried forward by a patch that does not change why the chart escaped (ADR-0018). The histogram reads the corpus scorer's records, never `spec_revisions`. **At P1 the library writes no escape reason at all** — no chart recipe is constructed, so the field has no instance; the bucket rides on the recorded planner output and the library raises instead (ADR-0019). **At P2, `generate_recipe` closes that gap** (ADR-0030): a well-formed bucket 1, 2 or 4 now constructs a `ChartRecipe` instead of raising, at `quality=` `balanced`/`best`. The library still raises — `InexpressibleRequestError` at `fast`, or as `generate_recipe`'s own terminal fallback when codegen or library resolution can't produce a usable document — and `UnanswerableInstructionError`/`PlannerFailureError` are untouched either way.
...
**Escalation**:
The gate writing bucket 4 and moving a deterministic chart onto the custom rail. The only trigger is a Flint `marks_present` fail, and only at `quality=` `balanced` or `best` — `fast` never escalates. A blank `painted` fail stays on-rail. Exhausted presentational repairs stay on-rail with `passed=False`; an `injection_pattern` fail never escalates. The failed frame's `transform`, `source_schema` and `theme_spec` carry onto the recipe; step 1 is not re-run. Remaining review-repair budget carries over; the recipe cannot escalate again. `generate_recipe` (ADR-0030) implements the hop; its own terminal failure — codegen or library resolution exhausted — still returns the Flint `ChartResult` with `passed=False`, the same fail-closed shape the stand-in used before the seam existed.
_Avoid_: treating every failed review as an escape; sending an injection fail onto generated JS; making rail share a function of the VLM; escalating a blank canvas; a fresh `quality=` budget after the hop; reading this loop as also switching `UnanswerableInstructionError` / `PlannerFailureError` (untouched) or bucket 3 (scorer-only) off their current outcomes — buckets 1 and 2 at `balanced`/`best` *do* call `generate_recipe` (ADR-0030)

**Review repair**:
An extra model ask after a failed review, charged to the per-request `quality=` budget (`fast` 0, `balanced` 1, `best` 2), never to the planner's emit cap. On Flint it is one step-2 call; on the custom rail it is one patch-prompt call carrying every failing name from that round (ADR-0026, ADR-0030). `colorblind_safe_palette` and `data_truthfulness` spend this budget on either rail; on custom rail every other repairable Tier-2 item — including `marks_present`, which carries no Flint-style carve-out there — spends it too (ADR-0030). `injection_pattern` never does (fail-closed). A **Flint** `marks_present` fail consumes none; on custom rail `marks_present` is repairable like any other name.
_Avoid_: sharing a cap with emit repairs; treating report-only as a second mode rather than budget 0; calling a **Flint** `marks_present` fail a repair (custom rail's is repairable); raising a review fail
...
_Avoid_: an escape field on `x_chartagent`, `ChartDocument` as the stored artifact, `bind_recipe` returning `BoundDocument`, a wire format for `BoundRecipe`, a sixth table, calling it a frame or a spec, a `semantic_types` field on `ChartRecipe` (they stay on the in-request `GeneratedRecipe`), typing `theme_spec` as `ThemePresetName` (a retired preset must stay constructable; the type is `str | ThemeSpec | None`)
...
**`generate_recipe`**:
The internal seam that returns a `GeneratedRecipe` — `document`, `transform` (supplied or authored), `semantic_types`, and code-computed `source_schema` — from which the caller assembles a `ChartRecipe` (ADR-0030). Never a factory kwarg, never `__all__`. `semantic_types` stay on that carrier. A miss authors them fresh on `RecipeDraft` (`transform`, `semantic_types`, `document`), drawn from the closed `SemanticTypeName` list. One function, two calling shapes: the bucket-4 hop supplies `transform` (carried over, not re-asked); a planner miss (bucket 1 or 2) leaves it unset and the same call authors one, on the same 8-slot-or-`raw_sql` grammar the frame uses. Reachable only at `quality=` `balanced`/`best` — `fast` never buys codegen, from either caller. Its own decode-retry budget is 1, disjoint from step 1's one retry, step 2's two, and the review-repair dial. The miss path's first ask is off the planner's 5-call cap; its retry stays on that cap. On exhaustion (codegen or library resolution) the hop returns the Flint `ChartResult` with `passed=False`; a miss raises `InexpressibleRequestError` as before the seam existed. Library names resolve through a private, unset-by-default `ChartAgent._library_resolver` (name, version) → (sha256, bytes) — the `_attempt_observer` convention, not a public kwarg — called synchronously so nothing that leaves `generate_recipe` ever carries an unresolved pin; resolver bytes stay internal to the request's own review-loop painting and never ride `ChartDocument`, `ChartRecipe` or `ChartResult`.
_Avoid_: a fourth `create_chart_agent` kwarg for the resolver, an unresolved `LibraryPin` crossing out of the seam, retrying a malformed patch response, importing Flint's `marks_present` carve-out onto custom rail
