...
**CheckResult**:
One check's outcome inside a `ReviewReport` — `name` (closed, widened per tier), `outcome` (`pass` | `fail` | `not_checked`), `detail`. One shape, unmodified across Tier 1/2/3; richer per-tier content (VLM rubric text, a DOM assertion) rides in `detail`, never a new field. `not_checked` means *this check can pass or fail here in principle, but couldn't be evaluated this run* (e.g. the `Rasteriser` extra is missing) — never *this check doesn't apply to this rail or backend*, which is instead simply absent from `ReviewReport.checks` (ADR-0024).
_Avoid_: a severity field, a fourth `outcome` for an internal error (those raise, never become a `CheckResult`), `not_checked` for a check that can never resolve pass/fail on this (rail, backend)

**ReviewReport**:
The review gate's frozen result type (ADR-0005): `tiers_run`, `tiers_skipped` (`unavailable` | `unsupported`, per tier), `passed` (scoped to tiers run), `budget_exhausted`, `checks: tuple[CheckResult, ...]`. Tier 1 always appears in `tiers_run` — `injection_pattern` alone guarantees a runnable check regardless of rasteriser availability or backend; only Tiers 2/3 ever land in `tiers_skipped` (ADR-0024).
_Avoid_: reading budget exhaustion's `passed=False` as a skip, treating an absent check as evidence the chart is fine, per-check severity as a substitute for `tiers_skipped`
...
**Chart agent**:
The public planner entry point: `create_chart_agent(model=...) -> ChartAgent`, then `agent.create_chart(data, instruction) -> ChartResult` — one call, profiling, both planner steps, and `bind` all run internally. Sync only; single-shot only, with no `history=` (needs a VFS, P2) and no `quality=` (needs the review gate, P2). Built by the planner build (ADR-0019/0020/0021/0022): `plan/` holds the two steps, the prompt renderer and backend selection, and the model client rides in a per-vendor extra.
_Avoid_: `sandbox=`/`outputs=`/`quality=`/`backend=` kwargs at P1 (nothing to configure, or the answer belongs to a later ticket), a reserved no-op kwarg for anything P2-only, treating `plan/` as public surface — `create_chart_agent` and `ChartAgent` are the only two names that leave it
