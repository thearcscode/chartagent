...
**`generate_recipe`**:
The internal seam that writes a `ChartDocument` (ADR-0030) — never a factory kwarg, never `__all__`. One function, two calling shapes: the bucket-4 hop supplies `transform` (carried over, not re-asked); a planner miss (bucket 1 or 2) leaves it unset and the same call authors one, on the same 8-slot-or-`raw_sql` grammar the frame uses. Reachable only at `quality=` `balanced`/`best` — `fast` never buys codegen, from either caller. Its own decode-retry budget is 1, disjoint from both the planner's 1/2/5 emit cap and the review-repair dial. On exhaustion (codegen or library resolution) the hop returns the Flint `ChartResult` with `passed=False`; a miss raises `InexpressibleRequestError` as before the seam existed. Library names resolve through a private, unset-by-default `ChartAgent._library_resolver` (name, version) → (sha256, bytes) — the `_attempt_observer` convention, not a public kwarg — called synchronously so nothing that leaves `generate_recipe` ever carries an unresolved pin; resolver bytes stay internal to the request's own review-loop painting and never ride `ChartDocument`, `ChartRecipe` or `ChartResult`.
