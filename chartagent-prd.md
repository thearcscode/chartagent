...
> **Erratum — 2026-09-05 ([#105](https://github.com/thearcscode/chartagent/issues/105), [ADR-0022](docs/adr/0022-the-prompt-renderer.md)).** *"Any model via LangChain"* is false: the model client is pydantic-ai, behind a per-vendor extra (ADR-0022 Decision 9). Self-hosted models and org chart conventions are unchanged.
...
> **Erratum — 2026-09-20 ([#170](https://github.com/thearcscode/chartagent/issues/170), [ADR-0029](docs/adr/0029-the-eval-benchmark-is-a-frozen-150.md)).** Goal 2's 95% is every-request **executable-output** on the frozen 150 (Flint: `painted` + row-count match; custom: harness non-blank on `Rasteriser` bytes). 85% is judge pass — five defects plus chart-type appropriateness — on **executable hits only**, excluded count beside. Judge family is distinct from the critic (`gemini-3.8-flash` while the critic is Anthropic). Working targets, not gates. The public 150 is tagged `eval-benchmark-v1` under `eval/`; hold-out is extra and unpublished.
...
- **The chart spec (the contract).** ~~A small, library-agnostic grammar of graphics of our own.~~ **Superseded by ADR-0002:** Flint's assembler argument *is* the spec — the **input frame** (`data` at render time, `semantic_types`, `chart_spec`, `options`, `theme_spec`), carrying our grammar in the single sibling key `x_chartagent` (`spec_version`, `transform`, `annotations`, `interactions`, `source_schema` — `escape` left the set with ADR-0018, taking `spec_version` to 1.2). Still versioned from day one, via `x_chartagent.spec_version`. See §8 and ADR-0002.
- **Renderers.** ~~A first-party `RendererAdapter` protocol with ECharts, Plotly and Matplotlib adapters, public so third parties can add their own.~~ **Superseded by ADR-0001:** we do not write renderers. Pinned **Flint** ships five backends — **Vega-Lite, ECharts, Chart.js, Plotly and Excel** — and compilation happens in the caller's client by loading Flint at `flint_version` and calling `assembleECharts` / `assembleVegaLite` / …. ~~ECharts remains the default web target.~~ **Superseded by [ADR-0021](docs/adr/0021-two-tier-backend-selection.md):** no backend is privileged that way — Studio carries no standing target and gets the same two-tier selection (a named `requested_backend`, else the ranking) any caller does; the ranking itself (ADR-0019) leads with Vega-Lite, not ECharts. **There is no Matplotlib backend** (see P0.4), and a public third-party adapter protocol is demoted to a P2 idea (§9 P2) — a second renderer stack means a second capability surface and a second fidelity suite, which is the exact cost ADR-0001 paid to avoid. What survives unchanged: **rendering receives materialized aggregate rows** (small by construction), not query handles; live-query handles stay a P2 idea.
...
> **Erratum — 2026-08-28 ([#57](https://github.com/thearcscode/chartagent/issues/57), [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md)).** **The erratum immediately above is withdrawn — it was a product error.** Closing the rail on a `matplotlib.figure.Figure` return makes the escape hatch produce a *worse* artifact than the rail it escapes: a static PNG where the deterministic rail gives an interactive browser chart. **Library choice is open, with no allowlist** — the agent picks from the request or writes the document from scratch (`libraries=()`) — and the PandasPlotBench figure concerns *Python* codegen, so it does not reach a web rail. The custom rail produces an **interactive web document** (HTML/JS/CSS) that Studio's browser runs with the bound rows and that review rasterises. **The phase ordering is also reversed: `web` is the phase-P2 profile and python is the later widening**, so the ADR-0015 D7 citation above no longer holds. Generated code is a module defining `render(data, el)` and `getPlottedSeries()` against a host-owned shell — not `make_chart(data) -> Figure`.
...
> **Erratum — 2026-09-18 ([#167](https://github.com/thearcscode/chartagent/issues/167), [ADR-0026](docs/adr/0026-tier-2-is-five-defect-checks-and-the-critique-is-a-leaf-call.md)).** The **Tier 2** bullet's rubric — *readability, truthfulness to data, chart-type appropriateness, aesthetics* — is **five defect checks**, one `CheckResult` each: `marks_present` plus the four Tier-1 lints ADR-0024 demoted (`axis_labels_present`, `legend_presence`, `label_overlap`, `bar_chart_y_axis_baseline`). *Truthfulness to data* stays with Tier 1 (ADR-0025), with `marks_present` as the picture's backstop; *chart-type appropriateness* and *aesthetics* are **not judged** by the gate. "Approve or emit structured feedback" is a typed verdict per item, and "bounded loop" is a **repair** of step 2's `chartProperties` on the deterministic rail (never the chart type, encodings, transform or backend) whose budget `quality` sets — `fast` is a budget of 0. Tier 2 runs only when the caller supplies a `Rasteriser` **and** a `critique_model`, which has no library default (§11/§12's "model per tier configurable"); otherwise it is skipped as `unavailable`. Tier 1's own narrowing is ADR-0024's.
...
> **Erratum — 2026-08-28 ([#57](https://github.com/thearcscode/chartagent/issues/57), [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md)).** The sequencing argument stands; the mechanism changes. House style is **not** `rcParams` — that runner is gone with ADR-0016 D1 — so the org brand palette rides the **channel as tokens**, and **skills still must not carry hex**. Phase-P2 guidance is **inline principles** (how to choose a library; when writing from scratch beats a dependency), with **no per-library catalogue**, because against an open library set a catalogue is an allowlist by another name.
...
> 4. **Rasterisation left, the `web` profile did not.** ADR-0003 moved *review-gate* rasterisation to a browser behind the `Rasteriser` protocol. The custom rail's `web` profile is untrusted generated JS and stays here. *(Phase corrected 2026-08-28, [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md) D2: the `web` profile is the **phase-P2** custom rail, not a §9 P1 fast-follow, and **python** is the later widening — the reverse of what this line originally said. It is also a browser rather than a sandbox image, and phase P2 builds no `SandboxBackend` at all, D3.)*
>
...
> **Erratum — 2026-08-28 ([#57](https://github.com/thearcscode/chartagent/issues/57), [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md)).** The mechanism changes with ADR-0016 D1's withdrawal: refresh is the **stored `ChartDocument` plus new rows delivered on the `postMessage` channel**, re-rendered in the sandboxed iframe — the host empties the container, calls `render`, then calls `getPlottedSeries`. Still not a trust exception, and still not free of infrastructure: it needs a browser rather than a Python sandbox.

> **Erratum — 2026-08-28 ([#60](https://github.com/thearcscode/chartagent/issues/60), [ADR-0018](docs/adr/0018-a-custom-rail-result-is-a-recipe-not-a-frame.md)).** The refresh box's custom-rail line is **`bind_recipe(recipe, data)`** — the stored `ChartRecipe`'s `transform` re-run for new rows, drift-checked against its `source_schema`, then delivered on the channel and re-rendered. It **never reads `module`, `styles` or `libraries`**, so a refresh cannot fail on the code and its failure modes are the deterministic rail's; the rails diverge only at paint. The patch box also narrows: *"targeted code edits, not rewrite"* means a recipe's review diff is **text** over `module` and `styles` and JSON over the rest — canonical JSON stays the content address for both rails but is not the diff here, since a canonical-JSON diff of a recipe is one changed line holding an entire program.
...
- ~~**`escape`** — `mode: custom_code` + `reason` (machine-recorded) + chosen `library` + `runtime_profile`. Whether it is a field inside `x_chartagent` or a sibling result type is still open (ADR-0002 *Related*).~~ **Superseded in all four parts by [ADR-0018](docs/adr/0018-a-custom-rail-result-is-a-recipe-not-a-frame.md) (2026-08-28, [#60](https://github.com/thearcscode/chartagent/issues/60)):** it is a **sibling result type**, and the `escape` key **leaves `x_chartagent`** (five keys; `spec_version` → 1.2). A frame was not merely unnecessary but *unconstructible* — one for a chart Flint never draws must still name a `chartType` from a closed generated `Literal`. The artifact is **`ChartRecipe`**: `spec_version` + `transform` + `source_schema` + `escape_reason` + `theme_spec` + a `ChartDocument`. `mode` is dropped (the type is the discriminator), `library` is dropped (it is `document.libraries`, pins with sha256s), and `runtime_profile` is dropped (P2 is web-only, ADR-0017 D2).
...
| P0.5  | Custom-code rail with the **`web`** runtime profile (**amended 2026-08-28, [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md) D2** — `python` is the later widening); generated code parameterized as a module defining `render(data, el)` + `getPlottedSeries()` against a host-owned shell                                          | Given a request the grammar can't express, the router records the reason; **generated code executes only inside the sandboxed iframe at an opaque origin — or, for review, inside the same inner iframe within the `Rasteriser`'s harness page — and never on Studio's own origin** (amended 2026-08-28, [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md) D3/D5; *"only in the sandbox"* described the python widening's `SandboxBackend`, which phase P2 does not build); re-invoking with new data requires no LLM call — **satisfiable as of 2026-08-28, [ADR-0018](docs/adr/0018-a-custom-rail-result-is-a-recipe-not-a-frame.md)**, which gives the code a home (`ChartRecipe`, stored in `spec_revisions` beside frames under a `kind` discriminator) and a verb (`bind_recipe`, which re-runs the stored transform and never reads the code) |
| P0.6  | Sandbox protocol + local backends (`bwrap` auto-detect → `subprocess` fallback) + `docker`. **Boundary kinds, not tiers ([ADR-0015](docs/adr/0015-the-sandbox-runs-programs-we-did-not-write.md) D11).**                                           | On Linux with user namespaces, `sandbox="local"` **executes** the bwrap probe (not a `PATH` check), selects it, reports `boundary == "os"`, and generated code cannot read host paths outside binds; on fallback `boundary == "none"` and exactly one `logging.warning` names implementation and boundary at construction; `require_isolation=True` raises `SandboxUnavailableError(kind="isolation_unmet")` at construction rather than falling back; docker backend reports `"os"`. **Amended 2026-08-28 ([ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md) D3): nothing here is built in phase P2.** The web custom rail needs no `SandboxBackend`, so this requirement is the contract for the **python widening** and is satisfied when that widening ships. Whether it remains a v1.0 must-have therefore depends on whether the python widening is in v1.0 — which [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md) did **not** decide, and which is recorded here rather than assumed either way |
...
> **Erratum — 2026-09-20 ([#170](https://github.com/thearcscode/chartagent/issues/170), [ADR-0029](docs/adr/0029-the-eval-benchmark-is-a-frozen-150.md)).** P0.10: smoke is **30 frozen IDs** from the public 150 (`balanced`, executable only). Nightly full-set is CI-only and **does not commit** to `main`. Published Goal 2/3 numbers are the pre-release (and any printed) run in `eval/report.md`. Human double-scoring is 30 executable hits per release; it does not rewrite 85%. Judge is a different **family** from the critic.
...
- Remaining pinned Flint backends promoted to first-class (Plotly, Chart.js, Excel). *(Amended 2026-08-28, [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md) D2: the `web` runtime profile is **no longer a fast-follow** — it is the **phase-P2** custom rail, and it is a browser rather than a Node-plus-headless-Chromium sandbox image. The **python** profile is the later widening; no §9 priority is assigned to it here.)*
...
> **Erratum — 2026-09-20 ([#170](https://github.com/thearcscode/chartagent/issues/170), [ADR-0029](docs/adr/0029-the-eval-benchmark-is-a-frozen-150.md)).** The executable/rubric line uses two denominators: 95% every-request on the frozen 150; 85% of executable hits. Published table is `eval/report.md`. Working targets, not gates.
...
| **P2 — agentic quality** | Custom-code rail (**`web` profile** — an interactive web document in a sandboxed iframe, [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md); *not* the python profile, and **no sandbox tiers**, which belong to the later python widening); review gate T1+T2; data-truthfulness check; quality dial; eval benchmark in CI | Quality signals surfaced in the product | P0.5–P0.8, P0.10 green; benchmark targets hit at `balanced` |
...
| Benchmark gamed by overfitting prompts to it                                                                                                                                    | Hold-out set rotated per release; community-contributed cases. **Amended 2026-09-20 ([#170](https://github.com/thearcscode/chartagent/issues/170), [ADR-0029](docs/adr/0029-the-eval-benchmark-is-a-frozen-150.md)):** the public 150 is **frozen**; hold-out is extra, unpublished, rotated by a pre-registered permutation; community cases append to the pool, never the frozen tag. |
...
| Benchmark judging                | ≥ 150 cases; judge model distinct from planner and critique VLM; ≥ 20% human double-scoring per release; published with confidence intervals (Goal 2, P0.10). **Specified by [ADR-0029](docs/adr/0029-the-eval-benchmark-is-a-frozen-150.md):** frozen 150 under `eval/`; judge is a different **family**; 85% is five defects plus appropriateness on executable hits; 20% is 30 executable hits and does not rewrite 85%. |
