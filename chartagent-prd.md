...
| P0.5  | Custom-code rail with the **`web`** runtime profile (**amended 2026-08-28, [ADR-0017](docs/adr/0017-the-custom-rail-produces-an-interactive-web-document.md) D2** — `python` is the later widening); generated code parameterized as a module defining `render(data, el)` + `getPlottedSeries()` against a host-owned shell                                          | Given a request the grammar can't express, the router records the reason; code executes only in the sandbox; re-invoking with new data requires no LLM call                                                                                                                                                                                                                      |
